"""
Model intelligence scores (single source of truth).

Keys are canonical model IDs (no suffix like :free).
Scores range from 0.0 to 1.0, representing relative model quality.
"""
...
SCORES: dict[str, float] = {
    ...
    "z-ai/glm4.7": 0.738,
    "z-ai/glm5": 0.778,
    "cogito-v1-preview": 0.51,
    "command-r7b-12-2024": 0.32,
    "databricks/dbrx-instruct": 0.38,
    "google/gemma-3-1b-it": 0.10,
    "google/gemini-2.0-flash": 0.68,
    "google/gemini-2.5-pro": 0.82,
    "huggingface/smollm-135m": 0.05,
    "inflection/inflection-3-pi": 0.44,
    "inflection/inflection-3-productivity": 0.46,
    "meta-llama/llama-3.2-1b-instruct": 0.08,
    "meta-llama/llama-3.2-3b-instruct": 0.12,
    "microsoft/phi-3-medium-128k-instruct": 0.28,
    "microsoft/wizardlm-2-8x22b": 0.36,
    "mistralai/mistral-7b-instruct-v0.3": 0.15,
    "mistralai/mistral-small-24b-instruct-2501": 0.28,
    "nousresearch/hermes-3-llama-3.1-405b": 0.44,
    "nousresearch/hermes-3-llama-3.1-70b": 0.38,
    "perplexity/sonar": 0.54,
    "perplexity/sonar-pro": 0.62,
    "perplexity/sonar-reasoning": 0.58,
    "qwen/qwen-2.5-72b-instruct": 0.42,
    "qwen/qwen-2.5-coder-32b-instruct": 0.46,
    "qwen/qwen-2.5-math-72b-instruct": 0.40,
    "snowflake/snowflake-arctic-instruct": 0.34,
    "thudm/glm-4-9b-chat": 0.22,
    "x-ai/grok-2-1212": 0.72,
    "x-ai/grok-beta": 0.65,
    "x-ai/grok-vision-beta": 0.60,
    "zero-one-ai/yi-large": 0.48,
    "zero-one-ai/yi-large-turbo": 0.42,
}


def canonicalize_model_id(model_id: str) -> str:
    """Remove suffix like :free from model ID."""
    if ":" in model_id:
        return model_id.rsplit(":", 1)[0]
    return model_id


def get_score(model_id: str) -> float | None:
    """Get quality score for a model."""
    canonical = canonicalize_model_id(model_id)
    return SCORES.get(canonical)
